...
<script>
  ...
  export default {
    ...
    created(){
      var that = this;
      this.$http
        .post(this.GLOBAL.rootUrl+'articles/otherarticlesbylike',{"userId":this.user.userId,"concerneduserId":this.id})
        .then(function (res) {
          that.popArticles=res.data.data;
        });
      this.$http
        .post(this.GLOBAL.rootUrl+'articles/otherarticlesbytime',{"userId":this.user.userId,"concerneduserId":this.id})
        .then(function (res) {
